...
contract TalkenStakingConfig is Initializable, AccessControlUpgradeable {
  ...
  bytes32 public constant TALKEN = keccak256("TALKEN");
  ...
  /// @notice Gets the Talken token address
  /// @return The address of the Talken token contract
  function getTalken() external view returns (address) {
    return _contractsMap[TALKEN];
  }
  ...
  /// @notice Updates the Talken token address
  /// @param talken The new Talken token address
  function updateTalken(address talken) external onlyRole(DEFAULT_ADMIN_ROLE) {
    _setContract({ key: TALKEN, val: talken });
  }
