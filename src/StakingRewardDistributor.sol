...
import { ITalken } from "./interfaces/ITalken.sol";
/**
...
contract StakingRewardDistributor is Initializable, Ownable2StepUpgradeable, ReentrancyGuardUpgradeable {
  ...
  /// @notice The TalkenStakingConfig contract instance
  TalkenStakingConfig public config;
  ...
  struct Init {
    ...
    /// @param config Address of the TalkenStakingConfig contract
    address config;
  }
  ...
  function initialize(Init memory init) public initializer {
    ...
    config = TalkenStakingConfig(init.config);
  ...
  function _checkpointToken() internal {
    ...
    uint256 rewardTokenBalance = IERC20(config.getTalken()).balanceOf(address(this));
  ...
  function claimTo(address recipient_) external nonReentrant onlyLive returns (uint256) {
    ...
    if (ITalken(config.getTalken()).isFrozen(recipient_)) {
  ...
  function _claimWithCustomRecipient(address user, address recipient_) internal returns (uint256) {
    ...
    if (total != 0) {
      ...
      IERC20(config.getTalken()).safeTransfer(recipient_, total);
    }
  ...
  function feed(uint256 amount) external nonReentrant onlyLive returns (bool) {
    IERC20(config.getTalken()).safeTransferFrom(msg.sender, address(this), amount);
  ...
  function kill() external onlyOwner nonReentrant {
    IERC20 rewardToken = IERC20(config.getTalken());
    isKilled = true;
